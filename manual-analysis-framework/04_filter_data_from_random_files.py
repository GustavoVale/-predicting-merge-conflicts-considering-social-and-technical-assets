...
def create_df_from_map(map):
    df = pd.DataFrame(columns=['file', 'occ', 'occ_conflicts'])
...
def save_df_with_unique_files_occurrence(df_sample, df_file, path_to_save, col_sample='ms_id', col_files='ms_id', target_col='filepath'):
    df = clean_df_file_names(df_sample,
